...
if (!empty($errors)) {
    ...
    header("Location: ../add_admin.php?" . $params);
    exit();
}
...
try {
    ...
    if ($stmt->execute()) {
        ...
        header("Location: ../admin_dashboard.php?success=" . urlencode("Admin account created successfully! Username: {$username}, Role: {$role}"));
        exit();
    } else {
        header("Location: ../admin_dashboard.php?error=" . urlencode("Failed to create admin account. Please try again."));
        exit();
    }
    
} catch (PDOException $e) {
    error_log("Database error in add_admin_handler.php: " . $e->getMessage());
    header("Location: ../admin_dashboard.php?error=" . urlencode("Database error occurred. Please try again."));
    exit();
} catch (Exception $e) {
    error_log("General error in add_admin_handler.php: " . $e->getMessage());
    header("Location: ../admin_dashboard.php?error=" . urlencode("An error occurred. Please try again."));
    exit();
}
?>
